...
_CATEGORY_RE = re.compile(
    r"^(?:category|change\s*category|switch\s*category|my\s*category)$",
    re.IGNORECASE,
)
...
def _layer1(message: str) -> ParseResult:
    ...
    if _CATEGORY_RE.match(stripped):
        return ParseResult(intent=TRADER_CATEGORY, confidence=1.0)
...
async def smart_parse_customer_message(
    ...
    *,
    ...
    prompt = (
        ...
        "Your job: understand the customer's INTENT by reading the conversation context carefully, "
        "then respond helpfully.\n\n"
        "IMPORTANT — READ THE CONVERSATION CONTEXT:\n"
        "- If you previously asked a question (budget, model, size, etc.), the customer's message "
        "  is likely ANSWERING that question. Interpret it in that context.\n"
        "- If you showed a numbered list, and the customer replies with a number, they mean that "
        "  option from YOUR list — NOT a quantity.\n"
        "- If you asked 'which storage?' and they reply '128', they mean 128GB.\n"
        "- If you asked 'what's your budget?' and they reply '400000', filter products under N400,000.\n"
        "- Always think: 'What did I just ask? How does their reply answer my question?'\n\n"
        "Return ONLY a JSON object — no markdown, no commentary.\n\n"
        ...
        "Actions:\n"
        "- action=order: customer clearly wants a SPECIFIC product. Match the EXACT catalogue name.\n"
        "  Set items with exact catalogue name and price from the catalogue above. NEVER set unit_price to 0.\n"
        "- action=clarify: customer asks about a product or wants to buy but there are multiple matches.\n"
        "  List ALL matching products from the catalogue as a NUMBERED LIST in this exact format:\n"
        "  1. Product Name - N123,000\n"
        "  2. Product Name - N456,000\n"
        "  ALWAYS use this numbered format (number + dot + name + dash + N + price).\n"
        "  Show ALL matches, not just a few. If customer asks for 'iphone 12 and iphone 14', list ALL\n"
        "  iPhone 12 variants AND all iPhone 14 variants in one numbered list.\n"
        "  End with a short question like 'Which would you like?' or 'Reply with the number to order.'\n"
        "  NEVER describe products in a paragraph — always use the numbered list.\n"
        "- action=negotiation: customer is haggling. Set offered_price if they name a specific amount.\n"
        "- action=payment_sent: customer says they've paid/sent/transferred.\n"
        "- action=greeting: saying hi or asking about the store. Reply warmly.\n"
        "- action=chitchat: social chat, non-order question. Reply warmly, don't start an order.\n"
        "- action=ignore: simple acknowledgment (ok, thanks, bye, cool). Do NOT reply.\n"
        "- action=unknown: can't determine intent. Ask what they'd like in 'reply'.\n\n"
        "Matching rules:\n"
        "- Match product names LOOSELY: 'iphone 14' → 'UK 14 Pro Max + eSIM'\n"
        "- '128' or '128gb' in context of a phone = 128GB storage variant\n"
        "- A budget number = filter products at or below that price\n"
        "- 'option 2' or just '2' after a numbered list = item #2 from that list\n"
        "- Yoruba numbers: meji=2, meta=3, ogun=20\n"
        "- Pidgin/informal: understand but reply in proper English\n"
        "- Keep 'reply' short, warm — max 2-3 sentences\n"
        "JSON:"
    )
